...
def drive_circle():
    goals = [   #Go in a straight line, i want to see it get there
                create_goal(-2, 0, 0, 0, 0, 0.0, 1.0),
                create_goal(random.uniform(-1.5,1.5), random.uniform(-2,2), 0, 0, 0, 0.539, 0.843),
...
if __name__ == '__main__':
    ...
    rospy.loginfo("Connected! Fixed Random Test initated!")
    startTime = time.time()
    drive_circle()
    endTime = time.time()
